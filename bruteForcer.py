...
def main():
	...
	startString = []
	startStringLen = list(range(len(startString)))
	alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
	
	print startStringLen
	...
	for i in range(nCr(26, args.noOfLetters)):
		for letter in alphabet:
			...
			if startString[-1] == "Z":
				for j, k in zip(reversed(startString), startStringLen):
					if (j == "Z") and (k  != args.noOfLetters):
						startString[k - 1] = chr(ord(startString[j] + 1))
						startString[j] = "A"
			print startString
